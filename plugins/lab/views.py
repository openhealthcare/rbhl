...
class LabMonthActivity(AbstractLabStatsPage):
    ...
    def get_results_rows(self, month, year):
        ...
        for blood in bloods:
            ...
            for result in blood.bloodresult_set.all():
                row = dict(blood_row)
                row["Allergen"] = result.allergen
                row["KU/L"] = result.kul
                row["IgE Class"] = result.klass
                row["RAST"] = result.rast
                row["RAST score"] = result.rast_score
                row["Precipitin"] = result.precipitin
                row["IgG"] = result.igg
                row["IgG Class"] = result.iggclass
                rows.append(row)
        return rows
    ...
    def post(self, *args, **kwargs):
        ...
        with ZipCsvWriter(zip_file_name) as zf:
            ...
            for employer in employers:
                ...
                zf.write_csv(
                    f"oem_{employer_name}_results_{month_name}_{year}.csv",
                    employer_results_rows
                )
