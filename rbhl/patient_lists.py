...
Column = partial(
    core.patient_lists.Column, limit=None, singleton=True, detail_template_path=None
)

ReferralSummary = Column(
    name="referral", title="Referral", template_path="columns/referral_summary.html"
)
Appointment = Column(
    name="appointment", title="Appointment", template_path="columns/appointment.html"
)

DemographicsColumn = Column(
    name="demographics", title="Demographics", template_path="columns/demographics.html"
)
...
class WithLetter(core.patient_lists.PatientList):
    ...
    schema = [DemographicsColumn, ReferralSummary, Appointment]

    def get_queryset(self, **kwargs):
        return (
            Episode.objects.exclude(letter=None)
            .exclude(peakflowday=None)
...
class ActivePatients(StaticTableList):
    ...
    def get_queryset(self):
        ...
        """
        return Episode.objects.filter(cliniclog__active=True).order_by(
            "cliniclog__clinicdate"
        )
